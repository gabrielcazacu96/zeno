...
`SelectField` accepts numeric values directly — pass `value={cat.id}` to
each `<SelectItem>` and declare the schema field as `z.number()`. Submit the
form below to see `categoryId` arrive as a number, not a string.
