...
accessibility wiring. Destructure from the `form` returned by `useZenoForm`,
...
`@zeno/forms` ships `<ValidationSpinner />` for the common case: a spinner
...
Wraps `@zeno/ui/radio-group`. `<RadioGroupFieldItem>` pairs each radio
...
Wraps `@zeno/ui/slider`. The form value is `number` for a single thumb,
...
Wraps `@zeno/ui/input-otp`. Defaults to a 6-slot group. Override with
...
Composes `@zeno/ui/popover` and `@zeno/ui/calendar`. The form value is
...
Wraps `@zeno/ui/combobox`. Single-select over a `string[]` with
...
|---|---|---|
| `items` | `string[]` | Options shown in the dropdown. Filtered client-side unless `filter={null}`. |
| `renderItem` | `(item: string) => ReactNode` | Override per-row rendering. |
