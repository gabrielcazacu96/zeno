...
Wraps `@zeno/ui/select`. Pass `<SelectItem>` children for the options. For
the common case of database rows where the ID is a number, see
[Select with numeric IDs](/docs/feature-modules/components/forms/examples#select-with-numeric-ids).
